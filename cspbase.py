...
from nonogram import colour_in
...
class CSP:
    ...
    def print_soln(self):
        ...
        if "Nonogram" in self.name:
            row_num = len(self.cons[0].get_scope())
            column_num = len(self.cons[len(self.cons)-1].get_scope())
            iteration = row_num
            solution2 = [0] * column_num
            k = 0
            w = 0
            solution2[k] = [0] * row_num
            for i, v in enumerate(self.vars):
                if i is 0 or (i+1) % (iteration) is not 0:
                    solution2[k][w] = v.get_assigned_value()
                    w += 1

                if (i+1)%(iteration) is 0:
                    solution2[k][w] = v.get_assigned_value()
                    k += 1
                    if k < column_num:
                        solution2[k] = [0] * row_num
...
class BT:
    ...
    def bt_search(self, propagator, var_ord, val_ord, tracer = None):
        ...
        if status == False:
            ...
            if tracer is None:
                ...
                status = self.bt_recurse(propagator, var_ord, val_ord, 1, tracer)  # now do recursive search
        ...
        if status == False:
            ...
            out_result = []
        if status == True:
            print("CSP {} solved. CPU Time used = {}".format(self.csp.name, time.process_time() - stime))
            out_result = self.csp.print_soln

        print("bt_search finished")
        self.print_stats()


        return out_result, time.process_time() - stime, self.nDecisions, self.nPrunings

    def bt_recurse(self, propagator, var_ord, val_ord, level, app = None):
        ...
        if not self.unasgn_vars:
            ...
            if var_ord.__name__ == "ord_custom":
                var = var_ord(self.csp, self.board)
            else :
            ...
            for val in value_order:
                ...
                '''
                put tracing method if you want
                '''

                # 1 means making them tuple black.
                if app is not None :
                    make_black = (var,)
                    draw_tracing(app, make_black, 1)
                ...
                if status:
                    if app is None :
                        ...
                        if self.bt_recurse(propagator, var_ord, val_ord, level + 1, app):
                            return True
                ...
                if app is not None :
                    make_white = (var,)
                    draw_tracing(app, make_white, 0)

            self.restoreUnasgnVar(var)
            return False

#If trace mode on, show the image every 1 sec.
def draw_tracing(app, vars, mode):

    for var in vars:
        if mode is 1:  # making them black
            if var.assignedValue is 1:
                color = 'black'
            else:
                color = 'white'
        else:
        ...
        comma = var.name.index(',')
        col = int(var.name[1:comma])
        row = int(var.name[comma+1:])

        w = Label(app.root, text='', bg=color, height=1, width=1)
        w.grid(row = app.ver_con_size + col  , column = app.hor_con_size + row )
